...
setwd("~/Documents/GitHub/Plasticity1")

#where was this calculated?
distances <- fread("movementData/distanceChanges.csv")

timing <- fread("timingData/finalTiming.csv", na.strings = "")
...
Classifying each year into tactics
...
animitation to show transition from dichotomy to full tactics, not used in report
...
Multiyear movements, identifying switches
...
```{r covariates, echo = F}
#removed sand creek, no longer in scope

y2y <- fread("movementData/updatedSwitchTable.csv")
...
looking into balance between tactics
...
taking lines from each of the tactics to use in figure
...
making switching figure

``` {r alluvial, echo = F, eval = F}
forAlluv <- y2y %>% group_by(strat1, strat2) %>% dplyr::summarise(n = n()) %>% filter(strat1 != strat2) 

forAlluv$strat1 <- factor(forAlluv$strat1, levels = c("R", "EM","SDM", "LDM"))

forAlluv <- forAlluv[order(forAlluv$strat1),]
...
alluv <- ggplot(forAlluv,
  ...
  labs(fill = "Strategy") +
  scale_fill_manual(values = c("#7570b3",   "#1b9e77","#e7298a", "#d95f02")) +
  geom_text(x = 1.1, y = 5, label = "10", family = "Century Gothic") +
  geom_text(x = 1.1, y = 14, label = "6", family = "Century Gothic") +
  geom_text(x = 1.1, y = 18, label = "1", family = "Century Gothic") +
  geom_text(x = 1.1, y = 22, label = "10", family = "Century Gothic") +
  geom_text(x = 1.1, y = 43, label = "27", family = "Century Gothic") +
  geom_text(x = 1.1, y = 57, label = "4", family = "Century Gothic") +
  geom_text(x = 1.1, y = 62, label = "10", family = "Century Gothic") +
  geom_text(x = 1.1, y = 82, label = "30", family = "Century Gothic") +
#  geom_text(x = 1.1, y = 78, label = "1", family = "Century Gothic") +
  geom_text(x = 1.1, y = 99, label = "1/1", family = "Century Gothic") +
  geom_text(x = 1.1, y = 102, label = "5", family = "Century Gothic")


ggsave("/Users/gzuckerman/Desktop/alluvial.jpeg", alluv, dpi = 600)


```


``` {r switch magnitude figure, echo = F, eval = F}
y2y$switchType <- paste0(y2y$strat1, " to ", y2y$strat2)


magnitude <- ggplot(y2y) + geom_segment(aes(x = 0, y = 0, xend = distanceDiff, yend= elevDiff), alpha = .2,
                                 arrow = arrow(length = unit(0.05, "inches"))) +
 
  geom_point(data = y2y %>% filter(strat1 == strat2), aes(x = distanceDiff, y = elevDiff), alpha = .1, color = "black") +
   geom_point(data = y2y %>% filter(strat1 != strat2), aes(x = distanceDiff, y = elevDiff, color = strat1), alpha = .5) +
  ...
  geom_text(x = 40000, y = 2000, label = "Increased distance and  \n elevation change \n (31.2%)", size = 3) +
  geom_text(x = 40000, y = -2400, label = "Increased distance and  \n decreased elevation change \n (19.2%)", size = 3) +
  geom_text(x = -40000, y = -2000, label = "Decreased distance and  \n elevation change \n (30.8%)", size = 3) +
  geom_text(x = -40000, y = 1000, label = "Decreased distance and  \n increased elevation change \n (18.8%)", size = 3) +
  theme_bw() + scale_color_manual("Switches From", values = c("#1b9e77", "#d95f02", "#7570b3", "#e7298a"))

ggsave("/Users/gzuckerman/Desktop/magnitude.jpeg", magnitude, dpi = 600)
...
```{r finding dist of tactics in each herd, eval = F, echo = F}

###This is where I can address arthurs comments

allYears <- rbind(y2y %>% dplyr::select(elk, year = year1, tactic = strat1, herd), 
                  y2y %>% dplyr::select(elk, year = year2, tactic = strat2, herd))

allYears <- allYears %>% distinct(elk, year, .keep_all= T)

tacticDist <- allYears %>% group_by(herd, tactic) %>% dplyr::summarise(count = n())
##key
herdTactDistSuppTable <- tacticDist %>% spread(tactic, count) %>% merge(
  tacticDist %>% group_by(herd) %>% summarise(total = sum(count))
)

write.table(herdTactDistSuppTable, file = "herdTactDistSuppTable.txt", sep = ",", quote = FALSE, row.names = F)



numberTactics <- tacticDist %>% group_by(herd) %>% dplyr::summarise(count = n()) %>% arrange(herd)
herdCounts <- allYears %>% group_by(herd) %>% dplyr::summarise(count = n()) %>% arrange(herd)

ggplot(tacticDist) + geom_bar(aes(fill = tactic, y = count, x = herd), position = "dodge", stat = "identity")

```

not enough data for resident model

```{r model glm AIC just resident, echo = F, eval = F}
#only six switches, so not enough to fit models
...
#dont run above code before running this
# 
# y2y$winter2feedAccess <- as.character(y2y$winter2feedAccess)
# y2y$winter2feedAccess <- as.factor(y2y$winter2feedAccess)
# y2y$winter2feedAccess <- relevel(y2y$winter2feedAccess, "0")
...
# 
# #log-likelihood
# logLik(model5$finalModel)
# 
# #examining residuals: non-significant residual pattern , basically normal
# testDispersion(model5$finalModel)
# 
...
#                         trControl = fitControl,preProcess = c('center', 'scale'))
# 
# #examining residuals: non-significant residual pattern, fairly normal
# testDispersion(model6$finalModel)
# 
...
# 
# #examining residuals: non-significant residual pattern, basically normal
# testDispersion(model7$finalModel)
# 
...
#only models with diversity are interactive the rest are additive
...
```{r model selection table for EM, echo = F, eval = F}
write.table(model.sel(model18$finalModel, model2$finalModel, model3$finalModel, model4$finalModel,
                        model5$finalModel, null),
            "emModSel.txt")
...
#log-likelihood
logLik(model18$finalModel)

#examining residuals: non-significant residual pattern, basically normal
testDispersion(model18$finalModel)
...
newdata1 <- with(data = model18$trainingData, 
                 data.frame(winter2propDev = seq(0,0.052, by = 0.001),
                                                         diversity = 0))
...
#low diversity
minDiv <-  with(data = model18$trainingData, data.frame(winter2propDev = seq(0,0.052, by = 0.001),
                                                         diversity = 0.5623351))

minDiv2 <- predict(preproc, minDiv)

minDiv2$`winter2propDev:diversity` <- minDiv2$winter2propDev*minDiv2$diversity


p <- predict(model18$finalModel, newdata = minDiv2,
             interval="confidence",se.fit = T, level = 0.85, type = "response")

minDiv$pred <- p$fit
minDiv$ll <- p$fit - 1.44 * p$se.fit
minDiv$ul <- p$fit + 1.44 * p$se.fit
minDiv$group <- "low" 


#high diveristy
maxDiv <-  with(data = model18$trainingData, data.frame(winter2propDev = seq(0,0.052, by = 0.001),
                                                         diversity = 1.3220876))

maxDiv2 <- predict(preproc, maxDiv)

maxDiv2$`winter2propDev:diversity` <- maxDiv2$winter2propDev*maxDiv2$diversity


p <- predict(model18$finalModel, newdata = maxDiv2,
             interval="confidence",se.fit = T, level = 0.85, type = "response")

maxDiv$pred <- p$fit
maxDiv$ll <- p$fit - 1.44 * p$se.fit
maxDiv$ul <- p$fit + 1.44 * p$se.fit
maxDiv$group <- "high" 

#plotting with all three lines
newdata1 <- rbindlist(list(newdata1, minDiv, maxDiv)) %>%
  mutate(ll = ifelse(ll<0, 0, ll), ul = ifelse(ul>1, 1, ul))

newdata1$group <- factor(newdata1$group, levels = c("low", "avg.", "high"), labels = c("Low", "Average", "High"))

emplot <- ggplot(newdata1 %>% rename(`Herd-level diversity` = group)) + 
  geom_line(aes(x = winter2propDev, y = pred, color = `Herd-level diversity`), alpha = .8) +
  geom_ribbon(aes(x = winter2propDev, y = pred, ymin = ll,ymax = ul, fill = `Herd-level diversity`), alpha = 0.2) +
  ylab("Probability of switching") + 
  xlab("Proportion of developed land in winter range") + 
  theme_bw() +  ylim(0, 1) + 
  scale_fill_brewer(palette = "Dark2") +
  geom_rug(data = y2y %>% filter(strat1 == "LDM", switch == 1), aes(winter2propDev), sides = "t") +
  geom_rug(data = y2y %>% filter(strat1 == "LDM", switch == 0), aes(winter2propDev), sides = "b") +
  scale_color_manual(values = c("#1b9e77", "#d95f02", "#7570b3"), guide = F)

ggsave("/Users/gzuckerman/Desktop/emplot.jpeg", emplot, dpi = 600)
...
#   theme_bw() + theme(legend.title = element_blank()) +
#   geom_rug(data = y2y %>% filter(strat1 == "EM"), aes(winter2propDev, color = switch)) + ylim(0, 1) +
#   scale_color_manual(values = c("#A8A8A8", "#000000"), guide = F)
...
div <- ggplot(newdata3) + 
  ...
  xlab("Herd-level diversity") + 
  theme_bw() + theme(legend.title = element_blank()) +
  geom_rug(data = y2y %>% filter(strat1 == "EM"), aes(diversity, color = switch)) + ylim(0, 1) +
  scale_color_manual(values = c("#A8A8A8", "#000000"), guide = F)
...
interaction <- ggplot(newdata5) + 
  ...
  geom_rug(data = y2y %>% filter(strat1 == "EM"), aes(winter2propDev, color = switch), sides = "b") +
  #this one not showing up
  geom_rug(data = y2y %>% filter(strat1 == "EM"), aes(y = diversity, color = switch), sides = "l") +
  scale_color_manual(values = c("#A8A8A8", "#000000"), guide = F)
...
```{r model selection table for SDM, echo = F, eval = F}
write.table(model.sel(model22$finalModel),
            "sdmModSel.txt")
...
#log-likelihood
logLik(model22$finalModel)

#examining residuals: non-significant residual pattern, basically normal
testDispersion(model22$finalModel)
...
ggplot(newdata5) + 
  ...
  xlab("Herd-level diversity") + 
  theme_bw() + theme(legend.title = element_blank()) +
  geom_rug(data = y2y %>% filter(strat1 == "SDM"), aes(diversity, color = switch)) + ylim(0, 1) +
  scale_color_manual(values = c("#A8A8A8", "#000000"), guide = F) 
...
```{r model selection table for LDM, echo = F, eval = F}
write.table(model.sel(model28$finalModel, model21$finalModel, model3$finalModel, model20$finalModel, null),
            "ldmModSel.txt")

```

```{r visualizing informative covs for LDM model 28, echo = F, eval = F}
y2y$winter2feedAccess <- as.character(y2y$winter2feedAccess)
y2y$winter2feedAccess <- as.factor(y2y$winter2feedAccess)
y2y$winter2feedAccess <- relevel(y2y$winter2feedAccess, "0")
y2y <- y2y %>% mutate(switch = ifelse(switch == T, 1, 0))
...
#log-likelihood
logLik(model28$finalModel)


#examining residuals: non-significant residual pattern, basically normal
testDispersion(model28$finalModel)

yardstick::mcc_vec(model28$trainingData$.outcome, predict(model28, model28$trainingData))


coef <- confint(model28$finalModel, level = .85) %>% as.data.frame() %>% 
  mutate(sig = ifelse((`7.5 %` < 0 & `92.5 %` < 0) | (`7.5 %` > 0 & `92.5 %` > 0), T, F))

preproc <- preProcess(y2y %>% filter(strat1 == "LDM") %>% dplyr::select(maxSnow, diversity),
                      method = c("center", "scale"))

#maxSnow
newdata1 <- with(data = model28$trainingData, data.frame(maxSnow = seq(0.31, 1.94, by = 0.01),
                                                         diversity = 0))

newdata2 <- predict(preproc, newdata1)

newdata2$diversity <- 0
newdata2$`maxSnow:diversity` <- 0


p <- predict(model28$finalModel, newdata = newdata2,
             interval="confidence",se.fit = T, level = 0.85, type = "response")
...
#low diversity
minDiv <-  with(data = model28$trainingData, data.frame(maxSnow = seq(0.31, 1.94, by = 0.01),
                                                         diversity = 0.5623351))

minDiv2 <- predict(preproc, minDiv)

minDiv2$`maxSnow:diversity` <- minDiv2$maxSnow*minDiv2$diversity


p <- predict(model28$finalModel, newdata = minDiv2,
             interval="confidence",se.fit = T, level = 0.85, type = "response")
...
#high diveristy
maxDiv <-  with(data = model28$trainingData, data.frame(maxSnow = seq(0.31, 1.94, by = 0.01),
                                                         diversity = 1.3220876))

maxDiv2 <- predict(preproc, maxDiv)

maxDiv2$`maxSnow:diversity` <- maxDiv2$maxSnow*maxDiv2$diversity


p <- predict(model28$finalModel, newdata = maxDiv2,
             interval="confidence",se.fit = T, level = 0.85, type = "response")
...
ldmplot <- ggplot(newdata1 %>% rename(`Herd-level diversity` = group)) + 
  geom_line(aes(x = maxSnow, y = pred, color = `Herd-level diversity`), alpha = .8) +
  geom_ribbon(aes(x = maxSnow, y = pred, ymin = ll,ymax = ul, fill = `Herd-level diversity`), alpha = 0.2) +
  ylab("Probability of switching") + 
  xlab("Maximum snow depth in winter range (m.)") + 
  theme_bw() +  ylim(0, 1) + scale_colour_brewer(palette = "Dark2") +
  scale_fill_brewer(palette = "Dark2") +
  geom_rug(data = y2y %>% filter(strat1 == "LDM", switch == 1), aes(maxSnow), sides = "t") +
  geom_rug(data = y2y %>% filter(strat1 == "LDM", switch == 0), aes(maxSnow), sides = "b") +
  scale_color_manual(values = c("#1b9e77", "#d95f02", "#7570b3"), guide = F)


ggsave("/Users/gzuckerman/Desktop/ldmplot.jpeg", ldmplot, dpi = 600)


#diversity
newdata3 <- with(data = model28$trainingData, data.frame(diversity = seq(0, 1.33, by = 0.01),
                                                         maxSnow = 0))

newdata4 <- predict(preproc, newdata3)

newdata4$maxSnow <- 0
newdata4$`maxSnow:diversity` <- 0

p <- predict(model28$finalModel, newdata = newdata4,
             interval="confidence",se.fit = T, level = 0.85, type = "response")
...
#shows interaction, just using this
diversity28 <- ggplot(newdata3) + 
  geom_line(aes(x = diversity, y = pred), alpha = .8) +
  geom_ribbon(aes(x = diversity, y = pred, ymin = ll,ymax = ul), alpha = 0.2) +
  ylab("Probability of switching") + 
  xlab("Herd-level movement tactic diversity") + 
  theme_bw() + theme(legend.position = "none") +
  geom_rug(data = y2y %>% filter(strat1 == "LDM"), aes(diversity, color = switch)) + ylim(0, 1) +
  scale_color_manual(values = c("#A8A8A8", "#000000"))
...
#maxSnow:diversity
grid <- expand.grid(seq(0, 1.33, by = 0.01), seq(0, 1.94, by = 0.01))


newdata5 <- with(data = model28$trainingData, data.frame(diversity = grid$Var1,
                                                         maxSnow = grid$Var2))

newdata6 <- predict(preproc, newdata5)

newdata6$`maxSnow:diversity` <- newdata6$maxSnow*newdata6$diversity

p <- predict(model28$finalModel, newdata = newdata6,
             interval="confidence",se.fit = T, level = 0.85, type = "response")
...
#more likely to switch at intermediately high snow depths with increased herd-level diversity!!
interaction <- ggplot(newdata5) + 
  geom_tile(aes(x = maxSnow, y = diversity, fill = pred)) +
  theme_bw() + xlab("Maximum snow depth in winter range (m.)") +
  ylab("Herd-level movement tactic diversity") +
  scale_fill_viridis_c(option = "B", direction = -1, name = "P(switch)", breaks = seq(.1, 1, by = .2)) +
  geom_rug(data = y2y %>% filter(strat1 == "LDM"), aes(maxSnow, color = switch), sides = "b") +
  geom_rug(data = y2y %>% filter(strat1 == "LDM"), aes(y = diversity, color = switch), sides = "l") +
  scale_color_manual(values = c("#A8A8A8", "#000000"), guide = F)
...
ggpubr::ggarrange(snow28, diversity28, interaction, labels = c("A.", "B.", "C."))


```

```{r visualizing informative covs for LDM model 21, echo = F, eval = F}
y2y$winter2feedAccess <- as.character(y2y$winter2feedAccess)
y2y$winter2feedAccess <- as.factor(y2y$winter2feedAccess)
y2y$winter2feedAccess <- relevel(y2y$winter2feedAccess, "0")
y2y <- y2y %>% mutate(switch = ifelse(switch == T, 1, 0))
...
#log-likelihood
logLik(model21$finalModel)


#examining residuals: non-significant residual pattern, basically normal
testDispersion(model21$finalModel)

yardstick::mcc_vec(model21$trainingData$.outcome, predict(model21, model21$trainingData))


coef2 <- confint(model21$finalModel, level = .85) %>% as.data.frame() %>% 
  mutate(sig = ifelse((`7.5 %` < 0 & `92.5 %` < 0) | (`7.5 %` > 0 & `92.5 %` > 0), T, F))

preproc <- preProcess(y2y %>% filter(strat1 == "LDM") %>% dplyr::select(maxSnow, springMigpropDev),
                      method = c("center", "scale"))

#max snow
newdata1 <- with(data = model21$trainingData, data.frame(maxSnow = seq(0, 1.94, by = 0.01),
                                                         springMigpropDev = 0))
...
p <- predict(model21$finalModel, newdata = newdata2,
             interval="confidence",se.fit = T, level = 0.85, type = "response")
...
snow21 <- ggplot(newdata1) + 
  geom_line(aes(x = maxSnow, y = pred), alpha = .8) +
  geom_ribbon(aes(x = maxSnow, y = pred, ymin = ll,ymax = ul), alpha = 0.2) +
  ylab("Probability of switching") + 
  xlab("Maximum snow depth in winter range (m.)") + 
  theme_bw() + theme(legend.title = element_blank()) +
  geom_rug(data = y2y %>% filter(strat1 == "LDM"), aes(maxSnow, color = switch)) + ylim(0, 1) +
  scale_color_manual(values = c("#A8A8A8", "#000000"), guide = F)


#spring dev
newdata3 <- with(data = model21$trainingData, data.frame(springMigpropDev = seq(0, 0.033, by = 0.0001),
                                                         maxSnow = 0))
...
p <- predict(model21$finalModel, newdata = newdata4,
             interval="confidence",se.fit = T, level = 0.85, type = "response")
...
springMigpropDev21 <- ggplot(newdata3) + 
  geom_line(aes(x = springMigpropDev, y = pred), alpha = .8) +
  geom_ribbon(aes(x = springMigpropDev, y = pred, ymin = ll,ymax = ul), alpha = 0.2) +
  ylab("Probability of switching") + 
  xlab("Proportion of developed land on spring migratory route") + 
  theme_bw() + theme(legend.title = element_blank()) +
  geom_rug(data = y2y %>% filter(strat1 == "LDM"), aes(springMigpropDev, color = switch)) + ylim(0, 1) +
  scale_color_manual(values = c("#A8A8A8", "#000000"), guide = F)

ggpubr::ggarrange(snow21, springMigpropDev21,labels = c("A.", "B."))


```

```{r visualizing informative covs for LDM model 3, echo = F, eval = F}
y2y$winter2feedAccess <- as.character(y2y$winter2feedAccess)
y2y$winter2feedAccess <- as.factor(y2y$winter2feedAccess)
y2y$winter2feedAccess <- relevel(y2y$winter2feedAccess, "0")
y2y <- y2y %>% mutate(switch = ifelse(switch == T, 1, 0))
...
#log-likelihood
logLik(model3$finalModel)


#examining residuals: non-significant residual pattern, basically normal
testDispersion(model3$finalModel)

yardstick::mcc_vec(model3$trainingData$.outcome, predict(model3, model3$trainingData))


coef <- confint(model3$finalModel, level = .85) %>% as.data.frame() %>% 
  mutate(sig = ifelse((`7.5 %` < 0 & `92.5 %` < 0) | (`7.5 %` > 0 & `92.5 %` > 0), T, F))

preproc <- preProcess(y2y %>% filter(strat1 == "LDM") %>% dplyr::select(springMigpropDev),
                      method = c("center", "scale"))

#spring dev
newdata1 <- with(data = model21$trainingData, data.frame(springMigpropDev = seq(0, 0.033, by = 0.0001)))
newdata2 <- predict(preproc, newdata1)
...
ggplot(newdata1) + 
  ...
  geom_ribbon(aes(x = springMigpropDev, y = pred, ymin = ll,ymax = ul), alpha = 0.2) +
  ylab("Probability of switching") + 
  xlab("Proportion of developed land in spring migratory route") + 
  theme_bw() + theme(legend.title = element_blank()) +
  geom_rug(data = y2y %>% filter(strat1 == "LDM"), aes(springMigpropDev, color = switch)) + ylim(0, 1) +
  scale_color_manual(values = c("#A8A8A8", "#000000"), guide = F)
...
```{r visualizing informative covs for LDM model 20, echo = F, eval = F}
y2y$winter2feedAccess <- as.character(y2y$winter2feedAccess)
y2y$winter2feedAccess <- as.factor(y2y$winter2feedAccess)
y2y$winter2feedAccess <- relevel(y2y$winter2feedAccess, "0")
y2y <- y2y %>% mutate(switch = ifelse(switch == T, 1, 0))
...
model20 <- train(switch ~ springMigpropDev+meanDFP, data = y2y %>% filter(strat1 == "LDM"),method = "glm",family = "binomial",
                        trControl = fitControl, preProcess = c('center', 'scale'))
#log-likelihood
logLik(model20$finalModel)


#examining residuals: non-significant residual pattern, basically normal
testDispersion(model20$finalModel)

yardstick::mcc_vec(model20$trainingData$.outcome, predict(model20, model20$trainingData))


coef <- confint(model20$finalModel, level = .85) %>% as.data.frame() %>% 
  mutate(sig = ifelse((`7.5 %` < 0 & `92.5 %` < 0) | (`7.5 %` > 0 & `92.5 %` > 0), T, F))

preproc <- preProcess(y2y %>% filter(strat1 == "LDM") %>% dplyr::select(meanDFP, springMigpropDev),
                      method = c("center", "scale"))


#spring dev
newdata3 <- with(data = model20$trainingData, data.frame(springMigpropDev = seq(0, 0.033, by = 0.0001),
                                                         maxSnow = 0))
...
p <- predict(model20$finalModel, newdata = newdata4,
             interval="confidence",se.fit = T, level = 0.85, type = "response")
...
k means 3 class justification

```{r k-means supp info 1, echo=FALSE}

forCluster <- nonRes %>% filter(elk %in% y2y$elk) %>%
  dplyr::select(dists, absElev) %>% mutate(dists = scale(dists, center = T),
                                                  absElev = scale(absElev, center = T))
...
clusterStats <- map_dfr(1:10, getClusterStats)
 ...
 ggplot(clusterStats) + geom_point(aes(num, within), color = "blue") + 
  ...
  scale_x_continuous(breaks = 1:10) + geom_vline(aes(xintercept = 2.225), linetype = "dashed") +
