...
#winter2propDev
newdata1 <- with(data = model14$trainingData, data.frame(maxSnow = seq(0, 1.94, by = 0.01),
                                                         diversity = 0))
...
snow28 <- ggplot(newdata1) + 
  geom_line(aes(x = maxSnow, y = pred), alpha = .8) +
  geom_ribbon(aes(x = maxSnow, y = pred, ymin = ll,ymax = ul), alpha = 0.2) +
  ylab("Probability of switching") + 
  xlab("Maximum snow depth in winter range (m.)") + 
  theme_bw() + theme(legend.title = element_blank()) +
  geom_rug(data = y2y %>% filter(strat1 == "LDM"), aes(maxSnow)) + ylim(0, 1)
...
diversity28 <- ggplot(newdata3) + 
  ...
  theme_bw() + theme(legend.title = element_blank()) +
  geom_rug(data = y2y %>% filter(strat1 == "LDM"), aes(diversity)) + ylim(0, 1)
...
interaction <- ggplot(newdata5) + 
  ...
  scale_fill_viridis_c(option = "B", direction = -1, name = "P(switch)", breaks = seq(.1, 1, by = .2)) +
  geom_rug(data = y2y %>% filter(strat1 == "LDM"), aes(maxSnow), sides = "b") +
  geom_rug(data = y2y %>% filter(strat1 == "LDM"), aes(y = diversity), sides = "l")
...
yardstick::mcc_vec(model21$trainingData$.outcome, predict(model28, model28$trainingData))


coef <- confint(model21$finalModel, level = .85) %>% as.data.frame() %>% 
  mutate(sig = ifelse((`7.5 %` < 0 & `92.5 %` < 0) | (`7.5 %` > 0 & `92.5 %` > 0), T, F))
...
springMigpropDev21 <- ggplot(newdata3) + 
  ...
  theme_bw() + theme(legend.title = element_blank()) +
  geom_rug(data = y2y %>% filter(strat1 == "LDM"), aes(springMigpropDev)) + ylim(0, 1)
...
```{r visualizing informative covs for LDM model 23, echo = F, eval = F}
