...
#models 2, 5, 6, 7: no 95% sig covs; 1 sig cov at 85% (model 5)
...
#diversity
newdata1 <- with(data = model5$trainingData, data.frame(diversity = seq(0, 20000, by = 100)))
...
#models 1, 2, 3, 4, 5, 6, 7, 15, no 95% or 85% sig covs
AICcmodavg::aictab(list(model1$finalModel, model2$finalModel, model3$finalModel, model4$finalModel,
                        model5$finalModel, model6$finalModel, model7$finalModel, model8$finalModel, 
                        model9$finalModel, model10$finalModel, model11$finalModel, model12$finalModel, 
                        model13$finalModel, model14$finalModel, model15$finalModel, model16$finalModel,
...
model15 <- train(switch ~ winter2propDev*winter2feedAccess, data = y2y %>% filter(strat1 == "EM"),
                 method = "glm",
                 family = "binomial",trControl = fitControl, preProcess = c('center', 'scale'))

coef <- confint(model15$finalModel, level = .85) %>% as.data.frame() %>% 
  mutate(sig = ifelse((`7.5 %` < 0 & `92.5 %` < 0) | (`7.5 %` > 0 & `92.5 %` > 0), T, F))

preproc <- preProcess(y2y %>% filter(strat1 == "EM") %>% select(winter2propDev, winter2feedAccess),
                      method = c("center", "scale"))

#winterDev with feed
newdata1 <- with(data = model15$trainingData, data.frame(winter2propDev = seq(0, 0.06, by = 0.001),
                                                        winter2feedAccess1 = 1))

newdata2 <- predict(preproc, newdata1)

newdata2$`winter2propDev:winter2feedAccess1` <- newdata2$winter2propDev
...
newdata3 <- with(data = model15$trainingData, data.frame(winter2propDev = seq(0, 0.06, by = 0.001),
...
newdata <- rbind(newdata1, newdata3) %>%  mutate(ll = ifelse(ll < 0, 0, ll), ul = ifelse(ul > 1, 1, ul))

ggplot(newdata) + 
  geom_line(aes(x = winter2propDev, y = pred, color = fed), alpha = .8) +
  geom_ribbon(aes(x = winter2propDev, y = pred, ymin = ll,ymax = ul, fill = fed), alpha = 0.2) +
  ylab("Probability of switching") + 
  xlab("Proportion of winter range in developed land") + 
  theme_bw() + labs(fill = "Fed", color = "Fed") +
  geom_rug(data = y2y %>% filter(strat1 == "EM") %>% 
             mutate(winter2feedAccess = ifelse(winter2feedAccess == 1, T, F)),
           aes(winter2propDev, color = winter2feedAccess))
...
#model 10: 1 sig cov at 95% and 85%
AICcmodavg::aictab(list(model1$finalModel, model2$finalModel, model3$finalModel, model4$finalModel,
                        model5$finalModel, model6$finalModel, model7$finalModel, model8$finalModel, 
                        model9$finalModel, model10$finalModel, model11$finalModel, model12$finalModel, 
                        model13$finalModel, model14$finalModel, model15$finalModel, model16$finalModel,
...
ggplot(newdata) + 
  ...
  theme_bw() + theme(legend.title = element_blank()) + 
  geom_rug(data = forRug, aes(dev, color = season))
...
ggplot(newdata5) + geom_tile(aes(x = winter2propDev, y = springMigpropDev, fill = pred)) +
  ...
  geom_rug(data = y2y %>% filter(strat1 == "LDM"), aes(springMigpropDev), sides = "l")
