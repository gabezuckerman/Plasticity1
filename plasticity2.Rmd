...
ggplot(newdata1 %>% rename(`Diversity level` = group)) + 
  geom_line(aes(x = winter2propDev, y = pred, color = `Diversity level`), alpha = .8) +
  geom_ribbon(aes(x = winter2propDev, y = pred, ymin = ll,ymax = ul, fill = `Diversity level`), alpha = 0.2) +
  ylab("Probability of switching") + 
  xlab("Proportion of developed land in winter range") + 
  theme_bw() +  ylim(0, 1) + 
  scale_fill_brewer(palette = "Dark2") +
  geom_rug(data = y2y %>% filter(strat1 == "LDM"), aes(winter2propDev, color = switch)) +
  scale_color_manual(values = c("#A8A8A8", "#000000", "#1b9e77", "#d95f02", "#7570b3"), guide = F)
...
ggplot(newdata1 %>% rename(`Diversity level` = group)) + 
  geom_line(aes(x = maxSnow, y = pred, color = `Diversity level`), alpha = .8) +
  geom_ribbon(aes(x = maxSnow, y = pred, ymin = ll,ymax = ul, fill = `Diversity level`), alpha = 0.2) +
  ylab("Probability of switching") + 
  xlab("Maximum snow depth in winter range (m.)") + 
  theme_bw() +  ylim(0, 1) + scale_colour_brewer(palette = "Dark2") +
  scale_fill_brewer(palette = "Dark2") +
  geom_rug(data = y2y %>% filter(strat1 == "LDM"), aes(maxSnow, color = switch)) +
  scale_color_manual(values = c("#A8A8A8", "#000000", "#1b9e77", "#d95f02", "#7570b3"), guide = F)
