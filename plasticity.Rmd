...
allIndYears <- allIndYears %>% merge(
  multibursts %>% distinct(elk = gps_sensors_animals_id, years = year, startDateYear) %>%
    mutate(dateYear = startDateYear + 1, elk = as.character(elk), years = as.character(years)) %>% dplyr::select(-startDateYear)
)


getYearlyHerdMPD <- function(h, y) {
  paste(h, y)
  allIndYears %>% filter(herd == h, dateYear == y) %>% dplyr::select(dists, elevs) %>% dist() %>% 
    as.numeric() %>% mean()
}

herdYears <- allIndYears %>% group_by(herd, dateYear) %>% summarise(n = n_distinct(elk)) %>% ungroup()

#how to deal with herd years that only have a single ind?
herdYears$mpd <- map2_dbl(herdYears$herd, herdYears$dateYear, getYearlyHerdMPD) 
